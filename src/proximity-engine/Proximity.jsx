...
const EASE_MAP = {
  ...
  swing: "back.inOut(3)",
  vibrate:
    "rough({ strength: 2, points: 20, template: 'none', taper: 'none', randomize: true })",
  robot: "steps(8)",
  ghost: "slow(0.6, 0.8, false)",
  expo: "expo.inOut",
  circus: "back.out(4)",
  glitch:
    "rough({ template: 'none', strength: 3, points: 50, taper: 'both', randomize: true })",
...
export const Proximity = ({
  ...
  useGSAP(() => {
    ...
    const setInitialState = () => {
      let initialProps = onReset
        ? onReset()
        : preset
          ? calculatePresetValues(preset, 0, config, true)
          : {};
      initialProps.willChange =
        "transform, filter, opacity, font-variation-settings";
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        items.forEach((item, i) => {
          ...
          if (isBlocked) {
            ...
            const dxEdge = Math.max(
              bounds.left - mouseX,
              0,
              mouseX - bounds.right,
            );
            const dyEdge = Math.max(
              bounds.top - mouseY,
              0,
          ...
          const intensity = Math.exp(
            -Math.pow(distance, falloff) / actualSpread,
          );
          ...
          const animationProps = onCalculate
            ? onCalculate(intensity, distance)
            : preset
              ? calculatePresetValues(preset, intensity, config, false)
              : {};
          const targetEase =
            EASE_MAP[config.ease] || config.ease || "power1.out";
    ...
    targetElement.addEventListener("touchstart", handleTouchMove, {
      passive: true,
    });
