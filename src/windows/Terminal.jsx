...
const fileSystem = {
  ".env": {
    type: "file",
    content:
      "hey hey wait! what are you searching for here ? huh ?! i knew you are trying to steal me so... here is some fake APIs\n\nAPI_KEY=ZED_SUPER_SECRET_12345\nDB_PASSWORD=password123 (so creative BTW, I bet you didn't expect it)\nSECRET_Salsa_Recipe=Add_More_Garlic (duh!)",
  },
  ".secret.txt": {
    type: "file",
    content:
  ...
  ".secrets.txt": {
    type: "file",
    content:
      "secret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nI LOVE YOU <3!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nI LOVE YOU <3!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nsecret!\nYo! Quit it no more secrets here\n",
  },
  "about_me.txt": {
    type: "file",
    content:
  ...
  certificates: {
    ...
    content: {
      ...
      "responsive_web.md": {
        type: "file",
        content: "freeCodeCamp Responsive Web Design\nCompleted: Jan 11, 2026",
      },
      "js_algorithms.md": {
        type: "file",
        content:
...
const Terminal = () => {
  ...
  const resolvePath = (target) => {
    ...
    let currentPath =
      target.startsWith("~") || target.startsWith("/") ? [] : [...cwd];
    const parts = target
      .replace(/^~?\/?/, "")
      .split("/")
      .filter(Boolean);
  ...
  const getSuggestion = () => {
    ...
    const dirPart =
      lastSlashIdx !== -1 ? target.substring(0, lastSlashIdx) : "";
    const filePart =
      lastSlashIdx !== -1 ? target.substring(lastSlashIdx + 1) : target;

    const { node } = resolvePath(dirPart);
    if (node && node.type === "dir") {
      const matches = Object.keys(node.content).filter((k) =>
        k.startsWith(filePart),
      );
      if (matches.length > 0) {
    ...
    if (SYSTEM_APPS[cmd]) {
      ...
      response = (
        <span className="text-blue-400">
          Launching {SYSTEM_APPS[cmd].name}...
        </span>
      );
    } else {
      switch (cmd) {
        case "help":
          response = (
            <div className="text-gray-400">
              ...
              menu <br />
              <span className="text-blue-400 font-bold">- ls [dir]:</span> List
              directory contents <br />
              <span className="text-blue-400 font-bold">- cd [dir]:</span>{" "}
              Change directory <br />
              <span className="text-blue-400 font-bold">- pwd:</span> Print
              working directory <br />
              <span className="text-blue-400 font-bold">
                - cat [file]:
              </span>{" "}
              Display file content in terminal <br />
              <span className="text-blue-400 font-bold">
                - kill [app]:
              </span>{" "}
              Kill app <br />
              ...
              Open file in graphical window <br />
              <span className="text-blue-400 font-bold">- whoami:</span> About
              the developer <br />
              <span className="text-blue-400 font-bold">- fastfetch:</span>{" "}
              System information <br />
              <span className="text-blue-400 font-bold">- clear:</span> Clear
              terminal output <br />
              <br />
              <span className="text-gray-500">Global Executables:</span>{" "}
              <span className="text-white">
                zen-browser, projects, gallery, contact, resume, code
              </span>
            </div>
          );
          break;
        ...
        case "ls": {
          ...
          const targetDir = args.find((a) => a !== "ls" && a !== "-a") || "";

          const { node } = resolvePath(targetDir);
          if (!node) {
            response = (
              <span className="text-red-400">
                ls: no such file or directory: {targetDir}
              </span>
            );
          } else if (node.type === "file") {
            response = <span className="text-white">{targetDir}</span>;
          } else {
            const keys = Object.keys(node.content).filter(
              (k) => showAll || !k.startsWith("."),
            );

            if (keys.length === 0 && !showAll) {
              response = (
                <span className="text-gray-500 italic">
                  No visible files in this directory. Try 'ls -a'
                </span>
              );
            } else {
              response = (
        ...
        case "cd": {
          ...
          if (!node) {
            ...
            response = (
              <span className="text-red-400">cd: not a directory: {arg}</span>
            );
          } else {
            setCwd(path);
            return;
        ...
        case "cat": {
          ...
          if (!node) {
            ...
            response = (
              <span className="text-red-400">cat: {arg}: Is a directory</span>
            );
          } else {
            if (arg === "techstack.md" || arg.endsWith("/techstack.md")) {
              ...
              response = (
                <span className="text-white whitespace-pre-wrap leading-relaxed">
                  {node.content}
                </span>
              );
            }
        ...
        case "xdg-open": {
          ...
          if (SYSTEM_APPS[arg]) {
            ...
            response = (
              <span className="text-blue-400">
                Launching {SYSTEM_APPS[arg].name}...
              </span>
            );
            break;
          }
          ...
          if (!node) {
            response = (
              <span className="text-red-400">
                xdg-open: {arg}: No such file or directory
              </span>
            );
          } else if (node.type === "dir") {
            response = (
              <span className="text-red-400">
                xdg-open: {arg}: Is a directory
              </span>
            );
          } else {
            ...
            if (fileName === "techstack.md") {
              response = (
                <span className="text-blue-400">
                  techstack.md is a CLI-only module. Use 'cat techstack.md'
                </span>
              );
            } else {
              openWindow("txtfile", {
        ...
        case "kill": {
          if (!arg) {
            response = (
              <span className="text-red-400">kill: missing operand</span>
            );
            break;
          }
          ...
          if (SYSTEM_APPS[targetApp]) {
            ...
            response = (
              <span className="text-blue-400">
                Killing {SYSTEM_APPS[targetApp].name}...
              </span>
            );
          } else {
            response = (
              <span className="text-red-400">
                kill: {arg}: process not found
              </span>
            );
          }
          break;
     ...
     |________/
              ...
              <div className="space-y-1 text-white">
                <p>
                  <span className="text-blue-500 font-bold">OS:</span> ZED OS
                  v6.7
                </p>
                <p>
                  <span className="text-blue-500 font-bold">Distro:</span>{" "}
                  CachyOS (Based on Arch BTW)
                </p>
                <p>
                ...
                <p>
                  <span className="text-blue-500 font-bold">Shell:</span> zsh
                  5.8
                </p>
                <p>
                ...
                <p>
                  <span className="text-blue-500 font-bold">CPU:</span> Intel
                ...
                <p>
                  <span className="text-blue-500 font-bold">GPU:</span> AMD R5
                  240 1GB VRAM
                </p>
              </div>
            </div>
          ...
          response = (
            <div className="flex flex-col">
              ...
              <span className="text-gray-500 italic mt-1">
                "You are so old... use{" "}
                <span className="text-white">fastfetch</span>, stupid! 🙄"
              </span>
            </div>
          );
          break;
          ...
          response = (
            <div className="flex flex-col space-y-1">
              ...
              <div className="text-red-400 mt-1 font-bold">
                Exaaaacttlllyyyy! You don't know it because I AM the super user
                here. 😎👑
              </div>
        ...
        default:
          response = cmd ? (
            <span className="text-red-400">zsh: command not found: {cmd}</span>
          ) : (
            ""
          );
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (cleanInput) {
      setHistory((prev) => [
        ...prev,
        { type: "command", content: input, dir: currentDirStr },
      ]);
      ...
      setHistory((prev) => [
        ...prev,
        { type: "command", content: "", dir: currentDirStr },
      ]);
    }
  };
        ...
        >
          ...
          <div className="relative flex-1 flex items-center h-[20px] overflow-hidden">
            <div className="absolute inset-0 pointer-events-none overflow-hidden flex items-center">
              <div
                ref={overlayRef}
                className="whitespace-pre font-terminal text-sm flex items-center w-max h-full"
              >
